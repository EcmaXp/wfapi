...
class WFBaseNode():
    ...
    __slots__  = ["id", "lm", "nm", "_ch", "no", "cp", "shared", "parent", "__weakref__"]
    slots = ["projectid", "last_modified", "name", "childs", "description", "completed", "last_completed", "shared", "parent"]
...
class WFNode(WFBaseNode):
    ...
    def __init__(self, id, lm=0, nm="", ch=None, no="", cp=None, shared=None, parent=None):
        self.projectid = id # UUID or "None"(DEFAULT_ROOT_NODE_ID)
        self.last_modified = lm # Last modified time
        self.name = nm # name
        self.ch = ch # children
        self.description = no # description
        self.last_completed = cp # Last completed time (or None)
        self.shared = shared # shared infomation
    ...
    def ch(self, childs):
        if hasattr(self, "_ch"):
            # if already _ch is exists, how to work?
            raise NotImplementedError
        elif childs is None:
            self._ch = None
        else:
    ...
    @description.setter(self, description)
    def description(self):
        assert isinstance(description, str)
        self.no = description
        
    @description.deleter(self)
    def description(self):
        self.no = None
    ...
    def __repr__(self):
        return "<{clsname}({id!r})>".format(clsname = type(self).__name__, id = self.id)
